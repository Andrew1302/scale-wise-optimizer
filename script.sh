...
# =========================
BASE_DIR="$(pwd)"
REPO_URL="https://github.com/EvolvingLMMs-Lab/lmms-eval.git"
REPO_NAME="lmms-eval"

PROJECT_PATH="${BASE_DIR}/${REPO_NAME}"

echo "📦 Indo para base dir"
mkdir -p "$BASE_DIR"
cd "$BASE_DIR"
...
echo "🧹 Removendo repo antigo (se existir)..."
rm -rf "$PROJECT_PATH"

if [ ! -d "$PROJECT_PATH" ]; then
    echo "🔽 Clonando lmms-eval..."
    git clone "$REPO_URL"
    cd "$PROJECT_PATH"

    echo "📌 Checkout commit compatível"
    git checkout 4b4049da17af9140442067aad5a1e2ddc1715c42
else
...
# REQUIREMENTS INLINE
# =========================
echo "🧾 Criando requirements inline..."

cat > requirements_frozen.txt << 'EOF'
accelerate==1.11.0
aiohttp==3.13.2
aiosignal==1.4.0
...
# =========================
echo "📁 Criando pasta train_seedbench"
mkdir -p train_seedbench
cd train_seedbench
# =========================
# CRIAR only.sh
# =========================
echo "📝 Criando only.sh"

cat > only.sh << 'EOF'
#!/bin/bash
set -e

RESOLUTIONS=(800000 2000 12500 25000 50000 100000 150000 250000 400000 600000)

MODEL_NAME="qwen2_vl"
MODEL_ARGS_BASE="pretrained=Qwen/Qwen2-VL-2B-Instruct"
BATCH_SIZE=1
...
BASE_PROJECT="/media/vm03/ssd1T/ic/lmms-eval"
BASE_DIR="/media/vm03/ssd1T/ic/lmms-eval/train_seedbench"

cd "$BASE_PROJECT"
export PYTHONPATH="$BASE_PROJECT:$PYTHONPATH"

export HF_HOME=/media/vm03/ssd1T/cache/hf
...
export TMPDIR=/media/vm03/ssd1T/cache/tmp

echo "Benchmarks: ${BENCHMARKS[@]}"
echo "Resoluções: ${RESOLUTIONS[@]}"
echo ""
...
for BENCHMARK in "${BENCHMARKS[@]}"; do
    ...
    for RES in "${RESOLUTIONS[@]}"; do
        ...
        OUTPUT_PATH="${BENCH_DIR}/${RES_NAME}"
        LOG_FILE="${BENCH_DIR}/${RES_NAME}.log"
        MODEL_ARGS="${MODEL_ARGS_BASE},min_pixels=1000,max_pixels=${RES}"

        echo "---- ${BENCHMARK} @ ${RES_NAME} ----"
...
cd ..
# =========================
# PATCH utils.py (TEMPLATE)
# =========================
echo "🧩 Patching utils.py"

UTILS_PATH="lmms_eval/tasks/mmmu/utils.py"

cat > "$UTILS_PATH" << 'EOF'
# ============================================
# CUSTOM MMMU UTILS PATCH
# Cole aqui o conteúdo completo do utils.py
# modificado.
#
# Este arquivo foi sobrescrito automaticamente
# pelo setup_lmms.sh
# ============================================

# >>>>>>>>>>>> COLE SEU TEMPLATE AQUI <<<<<<<<<<<<
import ast
import json
import os
import random
...
def mmmu_doc_to_visual(doc):
    ...
    # =========================
    log_path = "/media/vm03/ssd1T/ic/lmms-eval/train_seedbench/mmmu/multi_image.txt"
    ...
    # termino da mudança do chat gpt
...
# echo ""
# echo "Para rodar com tmux:"
# echo ""
# echo "cd $PROJECT_PATH/train_seedbench"
# echo "tmux new -s lmms_eval"
# echo "./only.sh"
# echo ""
# echo "Para sair sem matar:"
# echo "Ctrl + B  →  D"
# echo ""
# echo "Para voltar depois:"
# echo "tmux attach -t lmms_eval"
